...
def evaluate(model, criterion, tokenizer, eval_loader, device, input_lang, target_lang):
    ...
    return (
        ...
        running_chrf / len(eval_loader),
        running_bleu / len(eval_loader),
    )
...
if __name__ == "__main__":
    ...
    if args.short_run:
        ...
        dataset = dataset["train"].train_test_split(
            test_size=0.005, shuffle=True, seed=random_seed
        )
    ...
    for epoch in range(num_epochs):
        ...
        eval_loss, eval_chrf, eval_bleu = evaluate(
            model, criterion, tokenizer, eval_loader, device, input_lang, target_lang
        )
