...
def export_results(checkpoint_name, model_name, lang_pair, eval_chrf, eval_bleu, nickname, save_dir, eval_only_bool):
    
    out_json = {'checkpoint_name': checkpoint_name, 
            'base_model': model_name,
            'language_pair': lang_pair,
            'only_evaluation': eval_only_bool,
            'metrics':{'chrf': eval_chrf, 
                       'bleu': eval_bleu}}
    os.makedirs(f'experiment_results/{save_dir}', exist_ok=True)
    with open(f'experiment_results/{save_dir}/experiment_results_{nickname}.json', 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nickname", type=str, help="nickname for your experiement", default="lang_test_run"
    )
    parser.add_argument(
        "--experiment_nickname", type=str, help="nickname for your experiement", default="exp_test_run"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--how_many_samples", type=int, default=60000, help="how many samples to use out of all the dataset"
    )
    parser.add_argument(
        "--base_model_name",
        type=str,
    ...
    if len(dataset['train']) >= num_samples:
        ...
        dataset = dataset['train'].train_test_split(test_size=target_percentage, shuffle=True, seed=random_seed)
        dataset = dataset['test']
        dataset = dataset.train_test_split(test_size=0.16, shuffle=True, seed=random_seed)
    else:
...
#         dataset = dataset['train'].train_test_split(test_size=0.1, shuffle=True, seed=random_seed)
    ...
    if do_finetune:
        ...
        model_name_out = f"finetuned_checkpoints/{experiment_save_dir}/mt5_model_{lang_pair}.pth"
        if from_pt:
        ...
        export_results(curr_name_checkpoint, model_name, lang_pair, test_chrf, test_bleu, nickname, experiment_save_dir,only_eval)
    else:
        only_eval = True
        ...
        export_results(checkpoint_name, model_name, lang_pair, eval_chrf, eval_bleu, nickname, experiment_save_dir, only_eval)